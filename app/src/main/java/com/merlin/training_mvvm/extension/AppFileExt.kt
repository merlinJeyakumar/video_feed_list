package com.merlin.training_mvvm.extension

import android.content.Context
import android.os.Environment
import com.merlin.training_mvvm.R
import com.merlin.training_mvvm.model.FileModel
import java.io.File
...
fun getMediaDirectory(): File? {
    return File("${Environment.getExternalStorageDirectory()?.absolutePath}${File.separator}WhatsApp${File.separator}Media${File.separator}.statuses")
}

fun Context.getStoreDirectory(): File? {
    val saveDirectory = File(
        "${Environment.getExternalStorageDirectory()?.absolutePath}${File.separator}${this.getString(
            R.string.app_name
        )}"
    )
